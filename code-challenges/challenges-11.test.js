...
const count = (target, input) => {
  return input.reduce((accu,item) => { return ((item.filter(i=>i==target)).length+accu)},0)
};
...
const totalSum = (input) => {
  return input.reduce((accu,item)=>item.reduce((accu2,i)=> accu2+i)+accu,0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return (
   (input.map(item=>{
    return item.filter(i=> Number.isInteger(i) && i%5==0).map(i2=> Math.pow(2,i2))
...
let findMaleAndFemale = (data) => {
  let filteredArray= data.filter(item=> item.gender == 'female' || item.gender == 'male' );
  let char=''
  for (let i=0; i<filteredArray.length; ++i){   
    char += filteredArray[i].name
...
let findShortest = (data) => {
  return data.sort((accu,item)=> accu.height-item.height)[0].name
};
