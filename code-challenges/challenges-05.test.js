...
const addPurchases = (arr) => {
  let total= arr.reduce((accu,item)=>{    
    return (accu+item.purchasePrice);
  },0);
  return total;
};
...
const countNumberOfElements = (arr) => {
  let elementNum=arr.reduce((accu,item)=>{
    return accu+1;
  });
  return elementNum;
};
...
const returnNames = (arr) => {
  ...
  let nameArray=arr.reduce((accu,item)=>{
    accu.push(item.name); 
    return accu;   
  },[]);
  return nameArray;
...
const countNumberOfChildren = (arr) => {
  let result=arr.reduce((accu,item)=>{
    if (  item.children === undefined){
     console.log('this is undefined');
    }else{
      // accu=accu.concat(item.children);
      accu= accu.concat(item.children);       
    }  
    // accu=accu.concat(item.children); 
    return accu;
